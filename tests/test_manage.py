...
def test_manager(request, manager, consumer, battery, eval_data):
    ...
    simulator = enmanage.Simulator(manager, manager.predictor, consumer, battery)

    soc, budget, duty_cycle = simulator.run(eval_data["doy"], eval_data["e_in"])
...
def manager(request, battery, consumer, training_data, fxt_dataset):
    if request.param == "LT-ENO":
        ...
        e_in = fxt_dataset["data"]["exposure"].values[365:] * enmanage.PWR_FACTOR
        predictor = enmanage.CLAIRVOYANT(e_in)

        en_manager = enmanage.PREACT(
            predictor, battery.capacity, enmanage.profiles["uniform"]
        )

    elif request.param == "ST-EWMA":
        predictor = enmanage.EWMA()

        en_manager = enmanage.STEWMA(predictor, consumer)
