...
class Simulator(object):
    def __init__(self, manager, consumer, battery, dc_init=0.5, pwr_factor=1.0):
    ...
    @classmethod
    def plan_capacity(cls, doys, e_ins, latitude, config_path=None, config_dict={}):
        config = cls.get_config(config_path, config_dict)

        pwr_factor = cls.calc_pwr_factor(config_dict=config)

        astmodel = AST(
            training_data={"doy": doys, "e_in": e_ins * pwr_factor}, latitude=latitude
        )
        ...
        capacity_mah = (
            (surplus + deficit) / 2 / config["harvesting"]["voltage"] * 1000
    ...
    def from_config(
        ...
        capacity_wh = (
            config["battery"]["capacity_mah"] * config["harvesting"]["voltage"] / 1000
        )
        ...
        if consumer is None:
            consumer = Consumer(
                config["consumer"]["e_max_active"], config["consumer"]["e_baseline"]
            )
        ...
        if manager_cls is LTENO:
            ...
            manager = PREACT(
                predictor, battery.capacity, battery.get_age_rate(), **manager_args
            )
            ...
            manager = PIDPM(battery.capacity, battery.get_age_rate(), **manager_args)
        elif manager_cls == ENOMAX:
            predictor = None
        ...
        return cls(
            manager, consumer, battery, config["simulator"]["dc_init"], pwr_factor
        )
    ...
    def simulate_consumption(self, e_in, duty_cycle):
        ...
        duty_cycle_real = (e_out_real - self.consumer.consume(0.0)) / (
    ...
    def step(self, doy, e_in):
        ...
        self.next_duty_cycle = max(
            0.0,
            min(
                1.0,
                self.manager.calc_duty_cycle(doy, e_in_real, self.battery.get_soc()),
            ),
        )
